...
function generate_accesses_variables(accesses, variables, cacheline_size, array_size, variable_prefix)

  -- Offset is read once in each iteration
  local offset_addr = utils.align_addr(utils.get_var_address(variables, variable_prefix .. 'offset'), cacheline_size)
  utils.add_accesses(accesses, offset_addr, array_size, 0)

  -- Size is read once in each iteration + once after last
  local size_addr = utils.align_addr(utils.get_var_address(variables, variable_prefix .. 'size'), cacheline_size)
  ...
  local ptr_addr = utils.align_addr(utils.get_var_address(variables, variable_prefix .. 'ptr'), cacheline_size)
  ...
  local counter_addr = utils.align_addr(utils.get_var_address(variables, variable_prefix .. 'counter'), cacheline_size)
  utils.add_accesses(accesses, counter_addr, array_size * 4 + 1, array_size)

end

function generate_accesses(variables, cacheline_size, array_size, variable_prefix)
  ...
  -- Loop variables
  generate_accesses_variables(accesses, variables, cacheline_size, array_size, variable_prefix)
...
function test_suite(variables)
  ...
  accesses = generate_accesses(variables, cacheline_size, array_size, "foo_")

  utils.check_accesses(accesses, trace)

  -- Second test - correct access on the remote side.
  trace = utils.read_trace(tracer_output .. ".bar.0")
  
  accesses = generate_accesses(variables, cacheline_size, array_size / 2, "bar_")
  ...
  -- Third check on the host side
  trace = utils.read_trace(tracer_output .. ".host.2")
  ...
  if #json_cfg ~= 3 then 
    error("Expected three elements in JSON configuration, got " .. #json_cfg)
  end
  ...
  first_event = json_cfg[2]["after"]
  if first_event == nil then
    error("'After' key not found in the configuration")
  end

  second_event = json_cfg[3]["before"]
  if second_event == nil then
    error("'Before' key not found in the configuration")
  end
  ...
  for k, event in pairs(events) do
    ...
    region = event["region"]
    if region == nil or region ~= "bar" then
      error("Event " .. k .. ", expected region name 'bar', got " .. region)
    end

    counter = event["counter"]
    if counter == nil or counter ~= 0 then
      error("Event " .. k .. ", expected counter 0, got " .. counter)
    end
