...
function test_suite(variables)
  ...
  for iteration=0,size-1 do
    ...
    accesses = generate_accesses(variables, cacheline_size, array_size - iteration)
  ...
  -- For all iterations, except the last one, write (size - i) / 2
  -- For all iterations, except the first one, read (size - i) / 2
  ...
  for iteration=0,size-1 do
    ...
    if iteration < size - 1 then
      ...
      -- For writing, we take iteration number equal to the host trace number.
      -- The writing happens in current iteration, before kernel invocations.
      for i=0,(array_size-iteration)/2 - 1 do
    ...
    if iteration > 0 then
      ...
      -- For reading, we take iteration number equal larger by one than the host trace number.
      -- This is because reading happened in previous iteration, after kernel invocations.
      for i=0,(array_size - iteration + 1)/2 - 1 do
